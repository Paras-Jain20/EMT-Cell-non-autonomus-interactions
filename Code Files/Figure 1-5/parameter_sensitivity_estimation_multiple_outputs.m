...
for data_src_indx = 2%:length(data_src)
    for birth_tran_ratio = 250%[10 50 100 500 1000]
        for pop_model_indx = 33%[20 23 25 26 27] %[4 10 11 12 13 14]
...
%                 if(pre_samp == 1)
                    ...
                    % interpolated_timept =0:24*r1:30*24*r1; % interpolating time to get more data points to plot ODE solution trajectories

%                     cd('C:\Users\Asus\OneDrive - Indian Institute of Science\GitHub\EMT-State-Transition-2.0\code files ram\data post sampling')
% 
%                     updated_data = table2array(readtable(['data_post_iterative_sampling_Yamamoto_Exp_data_pd_mod_21_b_t_ratio_250_inverse_r1_54_inverse_r2_35']));
% 
%                     timept_original = sort(unique(data(:,1)))*24*r1;
%                     for time_indx = 1:length(timept_original)
%                         indx = find(abs((updated_data(:,1) - timept_original(time_indx))) > 10^-1);
%                         updated_data  = updated_data(indx,:);
%                     end
%                     additonal_timepts = sort(unique(updated_data(:,1)));


%                 else
%                     cd('C:\Users\Asus\OneDrive - Indian Institute of Science\GitHub\EMT-State-Transition-2.0\code files ram\data post sampling')
% %                     cd('C:\Users\Asus\OneDrive - Indian Institute of Science\GitHub\EMT-State-Transition-2.0\Manuscript documents\Revision\Parameter identifiability improvement')
% 
...
% %                     data = table2array(readtable(['data_post_iterative_sampling_pd_mod_21_b_t_250_r1_54_r2_35_output_2_total_cell_population']));
%                     original_num_timepts = 5;
%                     num_rep = 3;
%                     num_init_cond = 5; % number of different initial condition in the data
%                     init_cond = 1:num_init_cond;
%                     timept = sort(unique(data(:,1)));  %(data(1:(size(data,1)/(num_rep*num_init_cond)),1))'; % time is unitless as it is scaled by m cells division rate (r1)
%                     interpolated_timept = [0:24*r1:timept(end)]; % interpolating time to get more data points to plot ODE solution trajectories
...
%                         para_data = table2array(readtable(['profile_lik_post_samp_pd_mod_' num2str(pop_model_indx) '_b_t_' num2str(birth_tran_ratio) '_r1_' num2str(1/r1) '_r2_' num2str(inverse_r2) '_para_steps_' num2str(profile_sample_size) '_output_total_cell_population.csv']));
            ...
            for para_indx = para_start_indx:(size(para_data,2)-2)
                ...
                para_indx_step_left = ceil((left_indx-1-1)/num_para_plots);
                para_indx_step_right = ceil((length(x_rate)-right_indx+1)/num_para_plots);
                ...
                if(pop_model_indx < 15)
                    ...
                    ode_sim_data_outer_para_values = zeros(length(interpolated_timept)+1,length([1:para_indx_step_left:left_indx-1, right_indx+1:para_indx_step_right:length(x_rate)])*num_init_cond*num_rep);

                else
                    ode_sim_data = zeros(length(interpolated_timept),length(left_indx:para_indx_step:right_indx)*num_init_cond*num_rep);
                    ode_sim_data_outer_para_values = zeros(length(interpolated_timept),length([1:para_indx_step_left:left_indx-1, right_indx+1:para_indx_step_right:length(x_rate)])*num_init_cond*num_rep);
                end
                ...
                if(para_indx_step ~=0)
                    ...
                    for j = left_indx:para_indx_step:right_indx
                        ...
                        temp_ode_sim_data = ODE_simulation(pop_model_indx,num_rep,timept, interpolated_timept,interpolation,data,init_cond,original_num_timepts, x_rate_all(j,:)); % 1 is for num_rep of ODE simulated data

                        if(pop_model_indx > 15)
%                             temp_ode_sim_data = sum(temp_ode_sim_data(:,[3]),2)./sum(temp_ode_sim_data(:,3:4),2);
                            % temp_ode_sim_data = sum(temp_ode_sim_data(:,[1 3]),2)./sum(temp_ode_sim_data,2);
                            temp_ode_sim_data = sum(temp_ode_sim_data(:,[1:4]),2);

                        else
                            temp_ode_sim_data = temp_ode_sim_data(:,1)./sum(temp_ode_sim_data(:,1:2),2);
                        end
                        % concatenating the ODE solutions together for each increment in the para value for which profile likelihood is presently considered
                    ...
                    % for out of 95% confidence interval trajectories

                    itr_indx = 1; % as j deosn't starts at one.

                    for j = [1:para_indx_step_left:left_indx-1 right_indx+1:para_indx_step_right:length(x_rate)]
...
%                             temp_ode_sim_data = sum(temp_ode_sim_data(:,[3]),2)./sum(temp_ode_sim_data(:,3:4),2);
                        ...
                        % reshaping to timept row wise
                        if(pop_model_indx < 15)
                            ode_sim_data_outer_para_values(:,itr_indx:itr_indx + (num_rep*num_init_cond) -1) = [0.999 0.999 0.999 0.001 0.001 0.001; reshape(temp_ode_sim_data,length(interpolated_timept),num_init_cond*num_rep)];
                            %                         ode_sim_data(:,itr_indx:itr_indx + (num_rep*num_init_cond) -1) = [reshape(temp_ode_sim_data,length(interpolated_timept),num_init_cond*num_rep)];

                        else
                            ode_sim_data_outer_para_values(:,itr_indx:itr_indx + (num_rep*num_init_cond) -1) = reshape(temp_ode_sim_data,length(interpolated_timept),num_init_cond*num_rep);
                        end
                    ...
                    % plotting the ode trajectories for parameter values
                    % outside the 95% percent confidence interval

                    ini_dist_indx =1;
                    for itr_indx = 1:size(ode_sim_data_outer_para_values,2)
                        gcf = fig1;
                        subplot(num_init_cond,num_para, para_indx + num_para *(ini_dist_indx-1))
                        if(pop_model_indx < 15)
                            plot([0 interpolated_timept],ode_sim_data_outer_para_values(:,itr_indx),'Color','#808080','LineStyle','-');
                        else
                            plot([interpolated_timept],ode_sim_data_outer_para_values(:,itr_indx),'Color','#808080','LineStyle','-');
...
%                             plot(timept,data(num_rep*length(timept)*(ini_dist_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_dist_indx-1) + i*length(timept),2),'ko','MarkerSize',5);
%                     
...
%                         end
                        ...
                        % to plot the sampled experimental data on the ODE
                        % trajectories
                    
                        % cd('C:\Users\Asus\OneDrive - Indian Institute of Science\GitHub\EMT-State-Transition-2.0\Manuscript documents\Revision\Parameter identifiability improvement')
                        % data = table2array(readtable(['data_post_iterative_sampling_pd_mod_21_b_t_250_r1_54_r2_35_output_2_total_cell_population']));              
%                         original_num_timepts = 5;
%                         additonal_timepts = sort(unique(data(original_num_timepts*num_rep*num_init_cond+1:end,1)));
%                         for i = 1:num_rep
%                             % plotting experimental data one replicate at a time
%                             plot(additonal_timepts,data(original_num_timepts*num_rep*num_init_cond + num_rep*length(additonal_timepts)*(ini_dist_indx-1)+(i-1)*length(additonal_timepts)+1: original_num_timepts*num_rep*num_init_cond + num_rep*length(additonal_timepts)*(ini_dist_indx-1) + i*length(additonal_timepts),2),'bo','MarkerSize',7);
%                         
%                             %                     plot(timept/r1,data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),3),'bo','MarkerSize',5);
%                         end
...
%                         for i = 1:num_rep
%                             % plotting experimental data one replicate at a time
%                             plot(additonal_timepts,updated_data(num_rep*length(additonal_timepts)*(ini_dist_indx-1)+(i-1)*length(additonal_timepts)+1: num_rep*length(additonal_timepts)*(ini_dist_indx-1) + i*length(additonal_timepts),2),'bo','MarkerSize',5);
%                         
%                             %                     plot(timept/r1,data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),3),'bo','MarkerSize',5);
%                         end
                        ...
                        if(pop_model_indx < 15)
                            ...
                            % ax.YTick = 0:0.2:1;
                            % ax.XTick = (0:5:30)*24*r1;
                            % ax.XTickLabels = (0:5:30);
                            %                             ylim([0 1])
                            %                             xlabel('time (days)')
...
%                                         cd(directory)
            ...
            if(isempty(r2))
                ...
                if(pre_samp == 1)
%                     saveas(gcf,['Parameter Senstivity pre sampling pop model ' num2str(pop_model_indx) ' b t ratio ' num2str(birth_tran_ratio) ' inverse r1 ' num2str(1/r1) '_inverse_r2_' num2str(inverse_r2) '_para_steps_' num2str(profile_sample_size) '_EpCAM_low_fraction.png']);

                    saveas(gcf,['Parameter Senstivity pre sampling pop model ' num2str(pop_model_indx) ' b t ratio ' num2str(birth_tran_ratio) ' inverse r1 ' num2str(1/r1) '_inverse_r2_' num2str(inverse_r2) '_para_steps_' num2str(profile_sample_size) 'total cell numbers.png']);
                else
                    saveas(gcf,['Parameter Senstivity post sampling pop model ' num2str(pop_model_indx) ' b t ratio ' num2str(birth_tran_ratio) ' inverse r1 ' num2str(1/r1) '_inverse_r2_' num2str(inverse_r2) '_para_steps_' num2str(profile_sample_size) '_EpCAM_low_fraction.png']);

                    % saveas(gcf,['Parameter Senstivity post sampling pop model ' num2str(pop_model_indx) ' b t ratio ' num2str(birth_tran_ratio) ' inverse r1 ' num2str(1/r1) '_inverse_r2_' num2str(inverse_r2) '_para_steps_' num2str(profile_sample_size) 'total cell numbers.png']);
