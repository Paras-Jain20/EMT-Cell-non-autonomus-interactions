        ...
        for i = 1:num_rep
            if (interpolation == 0) % use experimental time points to plot ODE solutions
                if(pop_model_indx < 15)
                    ...
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),1),'r-','LineWidth',2) % plotting M number dynamics
                    hold on
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),2),'b-','LineWidth',2) % plotting E number dynamics
                elseif(pop_model_indx >= 15 && pop_model_indx < 28)
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),3)./(sum(ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),3:4),2)),'r-','LineWidth',2) % plotting M number dynamics
                    hold on
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),4)./(sum(ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),3:4),2)),'b-','LineWidth',2) % plotting E number dynamics
                else
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),1)./sum(ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),:),2),'r-','LineWidth',2) % plotting M number dynamics
                    hold on
                    plot(timept/r1,ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),2)./sum(ode_sim_data(num_rep*length(timept)*(ini_cond_indx-1)+(i-1)*length(timept)+1: num_rep*length(timept)*(ini_cond_indx-1) + i*length(timept),:),2),'b-','LineWidth',2) % plotting E number dynamics
                end
            else % use interpolated time points to plot ODE solutions

                if(pop_model_indx == 28 || pop_model_indx == 52)

                    plot(interpolated_timept/r1,sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),1:2),2),'r-','LineWidth',2) % plotting M number dynamics
                    hold on
                elseif(pop_model_indx == 51 || pop_model_indx == 53)

                    plot(interpolated_timept/r1,sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),1),2),'r-','LineWidth',2) % plotting M number dynamics
                    hold on

                elseif(pop_model_indx < 15)

                    plot(interpolated_timept/r1,ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),1),'r-','LineWidth',2) % plotting M number dynamics
                    hold on
                    %  plot(interpolated_timept/r1,ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),2),'b-','LineWidth',2) % plotting E number dynamics
                elseif(pop_model_indx >= 15)
...
%                     plotting EpCAM low fraction in Venus positive cell with time
if(pop_model_indx == 54)
    plot(interpolated_timept/r1,sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),4),2)./(sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),4:6),2)),'-','Color', Linecolor{2},'LineWidth',3)
else
    plot(interpolated_timept/r1,sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),3),2)./(sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),3:4),2)),'-','Color', Linecolor{2},'LineWidth',3)

end

                    hold on
% 
% %                     plotting EpCAM high fraction in Venus cells with time
%                     plot(interpolated_timept/r1,sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),[4]),2)./(sum(ode_sim_data(num_rep*length(interpolated_timept)*(ini_cond_indx-1)+(i-1)*length(interpolated_timept)+1: num_rep*length(interpolated_timept)*(ini_cond_indx-1) + i*length(interpolated_timept),3:4),2)),'-','Color', Linecolor{3},'LineWidth',3)
...
%             else
%                 saveas(gcf,['Cell number Yamamoto data Model Fit Ini Cond ' num2str(ini_cond_indx) ' pop mod ' num2str(pop_model_indx) ' b_t_ratio ' num2str(b_t_ratio)  ' r1 ' num2str(1/r1) ' para indx ' num2str(order_indx) '.png']);
%             end
%         else
%             if(pop_model_indx < 15)
...
%             else
%                 saveas(gcf,['Cell number Yamamoto data Model Fit Ini Cond ' num2str(ini_cond_indx) ' pop mod ' num2str(pop_model_indx) ' b_t_ratio ' num2str(b_t_ratio)  ' r1 ' num2str(1/r1) ' r2 ' num2str(inverse_r2)  ' para indx ' num2str(order_indx) '.png']);
%             end
%         end
%         cd(directory)
    end
